// ==========================================================================
// 🛡️ パスワード・セキュリティ（ガチャは不要とのことですがベース保護用）
// ==========================================================================
function vibrate() { if (navigator.vibrate) navigator.vibrate(15); }

// ==========================================================================
// 💾 グローバルセーブデータ構造 (初期化＆読み込み)
// ==========================================================================
let state = JSON.parse(localStorage.getItem('my_gacha_universe_state')) || {
    ...
    loginDays: 0,
    lastLoginDate: "",
    tickets: { ssr: 0, ur: 0, le: 0 }, // ランク分けされた確定チケット
...
window.addEventListener('DOMContentLoaded', () => {
    ...
    triggerPartnerSpeech(true); // 最初のセリフ
});
...
function triggerPartnerSpeech(isInitial = false) {
    ...
    // 確率で日替わり/システムメッセージか、カード固有セリフかを分岐
...
function renderGachaScreen() {
    ...
    if (currentGacha && currentGacha.isLocked) {
        ...
        actionControls.innerHTML = `
            <button onclick="pullGacha(1, false, true)" class="btn btn-gacha w-full">伝説の単発 (無料)</button>
            <button onclick="pullGacha(10, false, true)" class="btn btn-gacha-10 w-full">伝説の10連 (無料)</button>
        `;
    } else {
...
function renderCollection() {
    ...
    // 100%コンプかつ未ロックの場合のみ、伝説の殿堂入りボタンを解放
...
// ==========================================================================
// ☁️ GASクラウドセーブシステム（画像データを除く軽量軽量データのみ）
// ==========================================================================
function saveGasUrl() {
...
function getCleanStateForBackup() {
    // 画像データ(Base64)を含むとGASが破裂するため、構造だけを取り出したバックアップ用データを作る
    const cleanGachas = state.gachas.map(g => {
        return {
            id: g.id, title: g.title, isLocked: g.isLocked,
            cards: g.cards.map(c => ({ id: c.id, name: c.name, rarity: c.rarity, desc: c.desc, img: "LOCAL_STORED" })) // 画像はダミー文字に置換
        };
    });
    return {
        gachas: cleanGachas,
    ...
    if (shareData) {
        try {
            ...
            document.getElementById('btn-import-yes').onclick = () => {
                ...
                importedGacha.isLocked = false; // インポート時は未ロック状態にする
